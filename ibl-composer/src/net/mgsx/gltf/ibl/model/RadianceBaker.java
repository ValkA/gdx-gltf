...
public class RadianceBaker implements Disposable {
	...
	public Cubemap createRadiance(Cubemap cubemap, int baseSize, int sampleCount, boolean rgbm){
		...
		for(int level=0 ; level<mipMapLevels ; level++){
			...
			for(int s=0 ; s<6 ; s++){
				...
				renderSideRadiance(side, level, mipMapLevels+1, sampleCount, rgbm);
	...
	public Array<Pixmap> createPixmaps(Cubemap cubemap, int baseSize, int sampleCount, boolean rgbm){
		int mipMapLevels = GLUtils.sizeToPOT(baseSize) + 1;
		Pixmap[] maps = new Pixmap[mipMapLevels * 6];
		int index = 0;
		cubemap.bind();
	...
	private void renderSideRadiance(CubemapSide side, int level, int maxMipLevel, int sampleCount, boolean rgbm) {
		...
		float roughness = (float)level / (float)(maxMipLevel - 1);
	    shader.setUniformf("roughness", roughness);
		shader.setUniformi("u_sampleCount", sampleCount);
